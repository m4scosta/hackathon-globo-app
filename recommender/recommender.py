...
class Recommender:
    ...
    def set_keywords(self, keywords_list):
        ...
        pickle.dump(self.keywords, open(KEYWORDS_DUMP, "wb"))

    def get_keywords(self):
        if self.keywords is None:
            try:
                self.keywords = pickle.load(open(KEYWORDS_DUMP, "rb"))
    ...
    def get_content_vectors(self):
        if self.content_vectors is None:
            try:
                self.content_vectors = pickle.load(open(CONTENT_VEC_DUMP, "rb"))
            except pickle.PickleError:
    ...
    def get_user_preference_vectors(self):
        if self.user_preference_vectors is None:
            try:
                self.user_preference_vectors = pickle.load(open(USER_PREF_VEC_DUMP, "rb"))
            except pickle.PickleError:
    ...
    def save_user_preference_vector(self, user_preference_vectors):
        ...
        pickle.dump(self.user_preference_vectors, open(USER_PREF_VEC_DUMP, "wb"))

    def save_content_vectors(self, content_vectors):
        self.content_vectors = content_vectors
        pickle.dump(self.content_vectors, open(CONTENT_VEC_DUMP, "wb"))
    ...
    def add_content_vector(self, content_id, content_keywords):
        ...
        save_content_vectors(content_vectors)
